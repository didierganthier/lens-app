...
import { client, getProfiles, getPublications, getTimeline } from "../../api";
...
export default function Profile() {
  ...
  async function fetchProfile() {
    try {
      ...
      const timelineData = await client.query(getTimeline, { id }).toPromise();
      console.log({ timelineData });
      setTimeline(timelineData.data.timeline.items);
    } catch (error) {
      console.log({ error });
    }
  }
      ...
      <div>
        {
          timeline.map((time, index) => (
            <div key={index} style={{ padding: '20px', borderTop: '1px solid #ededed' }}>
              {time.type === 'publication' ? (
                <div>
                  <h4>{time.publication.title}</h4>
                  <p>{time.publication.body}</p>
                </div>
              ) : (
                <div>
                  <h4>{time.comment.publication.title}</h4>
                  <p>{time.comment.body}</p>
                </div>
              )}
            </div>
          ))
